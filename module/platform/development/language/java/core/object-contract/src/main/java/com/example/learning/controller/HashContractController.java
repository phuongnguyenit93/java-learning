...
public class HashContractController {
 static final class Broken { final int id; final int hash; Broken(int id,int hash){this.id=id;this.hash=hash;} public boolean equals(Object o){return o instanceof Broken b && id==b.id;} public int hashCode(){return hash;} }
 ...
 @GetMapping("/broken-contract") public Map<String,Object> brokenHashSetLookup(){ Set<Broken> set=new HashSet<>(); Broken stored=new Broken(1,101), equal=new Broken(1,202); set.add(stored); return Map.of("equals",stored.equals(equal),"storedHash",stored.hashCode(),"equalHash",equal.hashCode(),"containsEqualObject",set.contains(equal)); }
