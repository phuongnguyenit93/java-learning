...
│   └── portal-data/data/
│       └── module-catalog.json      # generated by ProjectStructureService
└── frontend/
    ├── package.json
    ├── vite.config.ts
    └── src/
...
Module hierarchy/routing hiện lấy từ generated `module-catalog.json`. `Overview` dùng metadata trong catalog; `Knowledge`, `Quiz`, `API Docs` hiện vẫn render fake TypeScript fixtures. Count ở sidebar/tabs được derive từ chính fake fixtures, không hard-code count riêng; count bằng `0` thì badge ẩn. `Execution` và những menu cần backend/runtime thật có thể giữ empty state trong phase đầu. CSS dùng stylesheet riêng; inline CSS không phải convention của Portal.
...
Real modules mode
→ giữ MODULE + các GROUP ancestor cần thiết
...
| Portal module hierarchy/catalog | filesystem/module discovery + module metadata via `ProjectStructureService` | `project-portal/src/main/resources/portal-data/data/module-catalog.json` |
| Portal current learning-content fixtures | `project-portal/frontend/src/data` fake TypeScript fixtures | Knowledge/Quiz/API Docs panels + derived counts |
...
34. Sidebar `Real modules` mode phải prune branch không chứa real module nhưng giữ GROUP ancestors cần thiết; không flatten toàn bộ real modules thành một list.
...
36. Real module navigation dùng action riêng (`>`); row có children sở hữu expand/collapse interaction.
...
generated module-catalog.json
        ↓
        + fake learning-content fixtures
        ↓
React + TypeScript
        ↓ Vite build
static bundle
